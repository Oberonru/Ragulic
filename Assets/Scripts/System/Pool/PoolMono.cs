...
namespace System.Pool
{
    ...
    public class PoolMono<T> where T : MonoBehaviour
    {
        ...
        public PoolMono(T prefab, Transform container, DiContainer diContainer, int count, bool autoExpand = false,
            bool activateAfterGetting = true)
        {
            if (prefab is null)
            {
        ...
        public PoolMono(T prefab, int count, Vector3 startPosition, bool autoExpand = false,
            bool activateAfterGetting = true)
        {
            if (prefab is null)
            {
        ...
        {
            T createdObject = DiContainer is null
                ? UnityEngine.Object.Instantiate(Prefab, Container)
                : DiContainer.InstantiatePrefabForComponent<T>(Prefab, Container);
